...
" target="_blank"> <img alt="header right" class="header_img_right" src="images/headcontact.png" style="width: 350px; height:127px;"> </a> </div> 
  ...
  <form method="POST" action="" accept-charset="UTF-8" name="formorder" id="update_form" enctype="multipart/form-data" class="form-vertical
validate_form" autocomplete="off" novalidate="novalidate">
    <input name="_token" type="hidden" value="OZYmqB9tPdy8ojsYzYrs4Pp3XDnhimmPaXx6Z62T">
    <div class="container mb-5">
      <div class="row bg-dark text-white">
        <div class="col-6 pt-3 pb-3"> Name </div>
        <div class="col-2 d-none d-sm-block  pt-3 pb-3"> Price in Rs. </div>
        <div class="col-3 col-sm-2 pt-3 pb-3"> Qty </div>
        <div class="col-3 col-sm-2  pt-3 pb-3"> Total(in Rs.) </div>
      </div>
      
  <?php   
$objResult = mysqli_query($mysqli,"SELECT * FROM tbl_category");
while($Fetch       = mysqli_fetch_array($objResult))
{
	$categoryid=$Fetch['Cid'];
  ?>
  
      
      <div class="row bgRed text-white">
        <div class="col-12  pt-2 pb-2"> <?php echo $Fetch['category']; ?> </div>
      </div>
       <?php   
$objResult1 = mysqli_query($mysqli,"SELECT * FROM tbl_product where categoryid='$categoryid'");
while($Fetch1       = mysqli_fetch_array($objResult1))
{
  ?>
      <div class="row border-1  pt-2 pb-2 align-items-center border">
        <div class="col-6 "> <a class="popup-link" href="images/gallery/<?php echo $Fetch1['product_image'];?>">
          <svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 0 24 24" width="24px" fill="#666">
           ...
           <?php echo $Fetch1['product']; ?> - <?php echo $Fetch1['um_type']; ?>
           
            
          <div class="d-inline d-sm-none"><br>
            - Rs. <del><?php echo $Fetch1['original_price']; ?></del><span class="text-danger pl-2 "><?php echo $Fetch1['offer_price']; ?></span></div>
          </a> </div>
        <div class="col-2 d-none d-sm-block"> <del><?php echo $Fetch1['original_price']; ?></del><span class="text-danger pl-2 "><?php echo $Fetch1['offer_price']; ?></span> </div>
        <div class="col-3 col-sm-2">
          <input type="hidden" name="price_<?php echo $Fetch1['Pid']; ?>" id="price_<?php echo $Fetch1['Pid']; ?>" value="<?php echo $Fetch1['offer_price']; ?>">
          <input type="hidden" name="products[]" id="products_<?php echo $Fetch1['Pid']; ?>" value="<?php echo $Fetch1['Pid']; ?>">
          
          
          
           
            <input type="hidden" name="category[]"  value="<?php echo $Fetch1['categoryid']; ?>">
             <input type="hidden" name="product[]"  value="<?php echo $Fetch1['product']; ?>">
              <input type="hidden" name="umtype[]"  value="<?php echo $Fetch1['um_type']; ?>">
              <input type="hidden" name="originalprice[]"  value="<?php echo $Fetch1['original_price']; ?>">
               <input type="hidden" name="offerprice[]"  value="<?php echo $Fetch1['offer_price']; ?>">
           
          <input type="number" name="qty[]" id="qty_<?php echo $Fetch1['Pid']; ?>" oninput="change_qty(<?php echo $Fetch1['Pid']; ?>)" class="form-control">
        </div>
        <div class="col-3 col-sm-2">
          <input type="number" name="total[]" id="total_<?php echo $Fetch1['Pid']; ?>" class="form-control" readonly="">
        </div>
      </div>
 <?php
}
...
?>
      ...
      <div class="row border-1  pt-2 pb-2 align-items-center border">
        <div class="col-6 col-sm-8 text-right"> <b>Total Amount</b> </div>
        <div class="col-3 col-sm-2 text-right">
          <input type="number" name="no_of_items" id="no_of_items" class="form-control" readonly="">
        </div>
        <div class="col-3 col-sm-2">
          <input type="number" name="total_amount" id="total_amount" class="form-control" readonly="">
        </div>
      </div>
      <div class="row row-eq-height  mt-5 pt-2 pb-2 align-items-center ">
        <div class="col-4 text-right  "> Your Name </div>
        <div class="col-8">
          <input class="form-control required" required placeholder="Enter Your Name" autocomplete="nope" name="name" type="text">
        </div>
      </div>
      <div class="row  pt-2 pb-2 align-items-center">
        <div class="col-4 text-right "> Your 10 Digit Mobileno </div>
        <div class="col-8">
          <input class="form-control required" required placeholder="Enter Your Mobile no" autocomplete="nope" maxlength="10" minlength="10" oninput="process(this)" name="mobile" type="text">
        </div>
      </div>
      <div class="row row-eq-height  mt-5 pt-2 pb-2 align-items-center ">
        <div class="col-4 text-right  "> Your Email Id </div>
        <div class="col-8">
          <input class="form-control required" required placeholder="Enter Your Email Id" autocomplete="nope" name="email" type="email">
        </div>
      </div>
      <div class="row   pt-2 pb-2 align-items-center">
        <div class="col-4 text-right"> Your Address </div>
        <div class="col-8">
          <textarea class="form-control required" required placeholder="Enter Your Address" autocomplete="nope" rows="3" name="address" cols="50"></textarea>
        </div>
      </div>
      <div class="row  pt-2 pb-2 align-items-center ">
        <div class="col-4 text-right "> Your State </div>
        <div class="col-8">
          <input class="form-control required" required placeholder="Enter Your State" autocomplete="nope" name="state" type="text">
        </div>
      </div>
      <div class="row   pt-2 pb-2 align-items-center">
        <div class="col-4 text-right"> Your City </div>
        <div class="col-8">
          <input class="form-control required" required placeholder="Enter Your City" autocomplete="nope" name="city" type="text">
        </div>
      </div>
      <div class="row   pt-2 pb-2 align-items-center">
        <div class="col-4 text-right"> Referal Code if Any </div>
        <div class="col-8">
          <input class="form-control" placeholder="Referal Code(If any othewise leave it as empty)" autocomplete="nope" name="referal_code" type="text">
        </div>
      </div>
      <div class="row   pt-2 pb-2 align-items-center">
        <div class="col-md-4 text-right"> </div>
        <div class="col-md-8">
          <button  class="btn btn-danger form-control" style="line-height: 40px;height:50px; background-color: #002366;
    border-color: #002366;" type="submit" name="Submit"  onclick="checkform()"><span></span>Submit Enquiry</button>
        </div>
      </div>
    </div>
  </form>
